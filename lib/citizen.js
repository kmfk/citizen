...
const runPublish = (namespace, name, provider, version, cmd) => {
  ...
  const tasks = new Listr([
    ...
    },
    {
      ...
      task: async (ctx) => {
        try {
          const res = await publish(registryAddr, moduleFullPath, ctx.tarball, owner, authToken);
          verbose(`response status code form registry: ${res.statusCode}`);
          if (res.statusCode >= 400) {
            throw new Error(`The registry server has returned an error: ${res.statusMessage}`);
          }
        } catch (err) {
          if (err.code === 'ECONNREFUSED') {
            throw new Error('No response from the registry server. Please check the registry.');
          }
          throw err;
        }
      },
...
commander
  ...
  .option('-a, --auth <auth>', 'Include an auth token')
